...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    public void onInitializeAccessibilityNodeInfoForItem(Recycler recycler, State state, View host, AccessibilityNodeInfo info) {
        ...
        if (lp instanceof LayoutParams) {
            ...
            if (this.mOrientation == 0) {
                ...
                info.setCollectionItemInfo(CollectionItemInfo.obtain(spanIndex, spanSize, spanGroupIndex, 1, z, false));
            } else {
                int spanIndex2 = glp.getSpanIndex();
                int spanSize2 = glp.getSpanSize();
                boolean z2 = this.mSpanCount > 1 && glp.getSpanSize() == this.mSpanCount;
                info.setCollectionItemInfo(CollectionItemInfo.obtain(spanGroupIndex, 1, spanIndex2, spanSize2, z2, false));
            }
            return;
        }
    ...
    public View findReferenceChild(Recycler recycler, State state, int start, int end, int itemCount) {
        ...
        for (int i = start; i != end; i += diff) {
            ...
            if (position >= 0 && position < itemCount && getSpanIndex(recycler, state, position) == 0) {
                if (((com.oneplus.lib.widget.recyclerview.RecyclerView.LayoutParams) view.getLayoutParams()).isItemRemoved()) {
    ...
    public void layoutChunk(Recycler recycler, State state, LayoutState layoutState, LayoutChunkResult result) {
        ...
        int maxSize = 0;
        while (true) {
            spanSize = Ints.MAX_POWER_OF_TWO;
            if (pos >= count) {
                break;
            }
            view2 = this.mSet[pos];
            ...
            spanSize = MeasureSpec.makeMeasureSpec(this.mCachedBorders[lp.mSpanIndex + lp.mSpanSize] - this.mCachedBorders[lp.mSpanIndex], Ints.MAX_POWER_OF_TWO);
            if (this.mOrientation == 1) {
                i = 0;
