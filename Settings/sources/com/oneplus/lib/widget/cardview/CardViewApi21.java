...
class CardViewApi21 implements CardViewImpl {
    ...
    public void initialize(CardViewDelegate cardView, Context context, int backgroundColor, float radius, float op_elevation, float maxElevation) {
        cardView.setBackgroundDrawable(new RoundRectDrawable(backgroundColor, radius));
        View view = (View) cardView;
        view.setClipToOutline(true);
        view.setElevation(op_elevation);
    ...
    public void setElevation(CardViewDelegate cardView, float op_elevation) {
        ((View) cardView).setElevation(op_elevation);
    }
    ...
    public void updatePadding(CardViewDelegate cardView) {
        if (cardView.getUseCompatPadding()) {
            float op_elevation = getMaxElevation(cardView);
            float radius = getRadius(cardView);
            int hPadding = (int) Math.ceil((double) RoundRectDrawableWithShadow.calculateHorizontalPadding(op_elevation, radius, cardView.getPreventCornerOverlap()));
            int vPadding = (int) Math.ceil((double) RoundRectDrawableWithShadow.calculateVerticalPadding(op_elevation, radius, cardView.getPreventCornerOverlap()));
