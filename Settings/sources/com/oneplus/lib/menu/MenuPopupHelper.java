...
public class MenuPopupHelper implements MenuHelper {
    ...
    public boolean tryShow(boolean forceUseStyleOffset) {
        return tryShow(false, forceUseStyleOffset);
    }

    public boolean tryShow(boolean useOffset, boolean forceUseStyleOffset) {
        if (isShowing()) {
            return true;
        }
        ...
        showPopup(0, 0, useOffset, false, forceUseStyleOffset);
        return true;
    }

    public boolean tryShow(int x, int y) {
        return tryShow(x, y, false);
    }

    public boolean tryShow(int x, int y, boolean forceUseStyleOffset) {
        if (isShowing()) {
            return true;
        }
        if (this.mAnchorView == null) {
        ...
        showPopup(x, y, true, true, forceUseStyleOffset);
        return true;
    }
    ...
    private MenuPopup createPopup() {
        ...
        if (Math.min(displaySize.x, displaySize.y) >= this.mContext.getResources().getDimensionPixelSize(R.dimen.op_abc_cascading_menus_min_smallest_width)) {
    ...
    private void showPopup(int xOffset, int yOffset, boolean useOffsets, boolean showTitle) {
        showPopup(xOffset, yOffset, useOffsets, showTitle, false);
    }

    private void showPopup(int xOffset, int yOffset, boolean useOffsets, boolean showTitle, boolean forceUseStyleOffset) {
        MenuPopup popup = getPopup();
        popup.setShowTitle(showTitle);
        if (useOffsets) {
            if ((GravityCompat.getAbsoluteGravity(this.mDropDownGravity, ViewCompat.getLayoutDirection(this.mAnchorView)) & 7) == 5 && !forceUseStyleOffset) {
                xOffset += this.mAnchorView.getWidth();
            }
            if (xOffset == 0) {
                xOffset = popup.getHorizontalOffset();
            }
