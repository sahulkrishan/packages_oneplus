...
public class ActivityPicker extends AlertActivity implements OnClickListener, OnCancelListener {
        ...
        public Drawable createIconThumbnail(Drawable icon) {
            ...
            try {
                ...
                if (iconWidth > 0 && iconHeight > 0) {
                    ...
                    if (width >= iconWidth) {
                        if (height >= iconHeight) {
                            if (iconWidth < width && iconHeight < height) {
                                Bitmap thumb = Bitmap.createBitmap(this.mIconWidth, this.mIconHeight, Config.ARGB_8888);
                    ...
                    Bitmap thumb2 = Bitmap.createBitmap(this.mIconWidth, this.mIconHeight, icon.getOpacity() != -1 ? Config.ARGB_8888 : Config.RGB_565);
                    Canvas canvas2 = this.mCanvas;
                    canvas2.setBitmap(thumb2);
                    this.mOldBounds.set(icon.getBounds());
                    y = (this.mIconWidth - width) / 2;
                    int y2 = (this.mIconHeight - height) / 2;
                    icon.setBounds(y, y2, y + width, y2 + height);
                    icon.draw(canvas2);
                    icon.setBounds(this.mOldBounds);
                    icon = new BitmapDrawable(thumb2);
                    ((BitmapDrawable) icon).setTargetDensity(this.mMetrics);
    ...
    protected static class PickAdapter extends BaseAdapter {
        ...
        public static class Item implements LabelledItem {
            ...
            public Intent getIntent(Intent baseIntent) {
    ...
    /* Access modifiers changed, original: protected */
    public Intent getIntentForPosition(int position) {
    ...
    /* Access modifiers changed, original: protected */
    public void putIntentItems(Intent baseIntent, List<Item> items) {
        PackageManager packageManager = getPackageManager();
        int i = 0;
