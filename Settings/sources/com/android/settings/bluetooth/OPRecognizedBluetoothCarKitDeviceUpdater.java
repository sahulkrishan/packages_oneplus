...
public class OPRecognizedBluetoothCarKitDeviceUpdater extends OPBluetoothCarKitDeviceUpdater implements OnPreferenceClickListener {
    private static final boolean DBG = true;
    private static final String TAG = "OPRecognizedBluetoothCarKitDeviceUpdater";
    private final AudioManager mAudioManager;
    private final BluetoothAdapter mBluetoothAdapter = BluetoothAdapter.getDefaultAdapter();

    public OPRecognizedBluetoothCarKitDeviceUpdater(Context context, DashboardFragment fragment, DevicePreferenceCallback devicePreferenceCallback) {
        super(context, fragment, devicePreferenceCallback);
        this.mAudioManager = (AudioManager) context.getSystemService("audio");
    }

    @VisibleForTesting
    OPRecognizedBluetoothCarKitDeviceUpdater(DashboardFragment fragment, DevicePreferenceCallback devicePreferenceCallback, LocalBluetoothManager localBluetoothManager) {
    ...
    public boolean isFilterMatched(CachedBluetoothDevice cachedDevice) {
        ...
        if (isDeviceConnected(cachedDevice)) {
            if (this.mBluetoothAdapter.isCarkit(cachedDevice.getDevice())) {
