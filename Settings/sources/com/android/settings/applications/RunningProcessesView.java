...
public class RunningProcessesView extends FrameLayout implements OnItemClickListener, RecyclerListener, OnRefreshUiListener {
    ...
    class ServiceListAdapter extends BaseAdapter {
        ...
        public boolean isEnabled(int position) {
            return ((MergedItem) this.mItems.get(position)).mIsProcess ^ 1;
        }
    ...
    public void refreshUi(boolean dataChanged) {
        ...
        synchronized (this.mState.mLock) {
            ...
            if (!(this.mCurTotalRam == totalRam && this.mCurHighRam == highRam && this.mCurMedRam == medRam && this.mCurLowRam == lowRam)) {
                ...
                this.mAppsProcessText.setText(getResources().getString(R.string.running_processes_header_ram, new Object[]{sizeStr2}));
                sizeStr2 = bidiFormatter.unicodeWrap(Formatter.formatShortFileSize(getContext(), highRam));
                this.mForegroundProcessText.setText(getResources().getString(R.string.running_processes_header_ram, new Object[]{sizeStr2}));
                int progress = (int) ((((float) highRam) / ((float) totalRam)) * 1120403456);
                this.mColorBar.setProgress(progress);
                this.mColorBar.setSecondaryProgress(((int) ((((float) medRam) / ((float) totalRam)) * 100.0f)) + progress);
            }
        }
    }
    ...
    private void startServiceDetailsActivity(MergedItem mi) {
        if (this.mOwner != null && mi != null) {
            ...
            new SubSettingLauncher(getContext()).setDestination(RunningServiceDetails.class.getName()).setArguments(args).setTitle((int) R.string.runningservicedetails_settings_title).setSourceMetricsCategory(this.mOwner.getMetricsCategory()).launch();
        }
    }
