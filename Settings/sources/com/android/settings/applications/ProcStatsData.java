...
public class ProcStatsData {
    ...
    private void distributeZRam(double zramWeight) {
        ...
        for (i = this.pkgEntries.size() - 1; i >= 0 && totalTime > 0; i--) {
            ...
            if (pkgZRam > 0) {
                ...
                ProcStatsEntry procEntry = new ProcStatsEntry(entry2.mPackage, 0, this.mContext.getString(2131889838), maxRunTime, pkgZRam, this.memTotalTime);
    ...
    private ProcStatsPackageEntry createOsEntry(ProcessDataCollection bgTotals, ProcessDataCollection runTotals, TotalMemoryUseCollection totalMem, long baseCacheRam) {
        ...
        if (totalMemoryUseCollection.sysMemNativeWeight > 0.0d) {
            String str = Utils.OS_PKG;
            String string = this.mContext.getString(R.string.process_stats_os_native);
            String str2 = string;
            ProcStatsEntry procStatsEntry2 = new ProcStatsEntry(str, 0, str2, this.memTotalTime, (long) (totalMemoryUseCollection.sysMemNativeWeight / ((double) this.memTotalTime)), this.memTotalTime);
            packageManager = this.mPm;
            procStatsEntry2.evaluateTargetPackage(packageManager, this.mStats, bgTotals, runTotals, sEntryCompare, this.mUseUss);
            osPkg.addEntry(procStatsEntry2);
        }
        if (totalMemoryUseCollection.sysMemKernelWeight > 0.0d) {
            procStatsEntry = new ProcStatsEntry(Utils.OS_PKG, 0, this.mContext.getString(R.string.process_stats_os_kernel), this.memTotalTime, (long) (totalMemoryUseCollection.sysMemKernelWeight / ((double) this.memTotalTime)), this.memTotalTime);
            packageManager = this.mPm;
            procStatsEntry.evaluateTargetPackage(packageManager, this.mStats, bgTotals, runTotals, sEntryCompare, this.mUseUss);
            osPkg.addEntry(procStatsEntry);
        }
        if (baseCacheRam > 0) {
            procStatsEntry = new ProcStatsEntry(Utils.OS_PKG, 0, this.mContext.getString(R.string.process_stats_os_cache), this.memTotalTime, baseCacheRam / PlaybackStateCompat.ACTION_PLAY_FROM_MEDIA_ID, this.memTotalTime);
            packageManager = this.mPm;
            procStatsEntry.evaluateTargetPackage(packageManager, this.mStats, bgTotals, runTotals, sEntryCompare, this.mUseUss);
            osPkg.addEntry(procStatsEntry);
        }
        ...
        for (ipkg = 0; ipkg < N; ipkg++) {
            ...
            for (int iu = 0; iu < pkgUids.size(); iu++) {
                ...
                for (int iv = 0; iv < vpkgs.size(); iv++) {
                    ...
                    for (iproc = 0; iproc < st.mProcesses.size(); iproc++) {
                        ...
                        if (proc == null) {
                            ...
                            stringBuilder.append("No process found for pkg ");
                            stringBuilder.append(st.mPackageName);
                            stringBuilder.append("/");
                            stringBuilder.append(st.mUid);
                            stringBuilder.append(" proc name ");
                            stringBuilder.append(pkgProc.getName());
                            Log.w(str, stringBuilder.toString());
                        } else {
                            ProcStatsEntry ent = (ProcStatsEntry) entriesMap.get(proc.getName(), proc.getUid());
                            if (ent == null) {
                                String str2 = st.mPackageName;
                                ent = new ProcStatsEntry(proc, str2, bgTotals, runTotals, procStatsData.mUseUss);
