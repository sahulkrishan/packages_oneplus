...
public final class InetAddresses {
    ...
    public static final class TeredoInfo {
        ...
        public TeredoInfo(@Nullable Inet4Address server, @Nullable Inet4Address client, int port, int flags) {
            ...
            boolean z2 = flags >= 0 && flags <= 65535;
    ...
    private static byte[] ipStringToBytes(String ipString) {
        ...
        for (int i = 0; i < ipString.length(); i++) {
            ...
            if (c == '.') {
                hasDot = true;
            } else if (c == AccessibilityUtils.ENABLED_ACCESSIBILITY_SERVICES_SEPARATOR) {
                if (hasDot) {
                    return null;
                }
                hasColon = true;
    ...
    private static String hextetsToIPv6String(int[] hextets) {
        ...
        for (int i = 0; i < hextets.length; i++) {
            ...
            if (thisIsNumber) {
                if (lastWasNumber) {
                    buf.append(AccessibilityUtils.ENABLED_ACCESSIBILITY_SERVICES_SEPARATOR);
                }
