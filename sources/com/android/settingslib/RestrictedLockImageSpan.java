...
public class RestrictedLockImageSpan extends ImageSpan {
    ...
    public void draw(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y, int bottom, Paint paint) {
        ...
        canvas.save();
        canvas.translate(this.mExtraPadding + x, ((float) (bottom - drawable.getBounds().bottom)) / 2.0f);
        drawable.draw(canvas);
        canvas.restore();
    }

    public int getSize(Paint paint, CharSequence text, int start, int end, FontMetricsInt fontMetrics) {
        return (int) (((float) super.getSize(paint, text, start, end, fontMetrics)) + (2.0f * this.mExtraPadding));
    }
}
