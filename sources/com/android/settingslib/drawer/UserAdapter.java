...
public class UserAdapter implements SpinnerAdapter, ListAdapter {
    ...
    public static class UserDetails {
        ...
        public UserDetails(UserHandle userHandle, UserManager um, Context context) {
            ...
            if (userInfo.isManagedProfile()) {
                ...
                icon = context.getDrawable(17302325);
            } else {
    ...
    public static UserAdapter createUserSpinnerAdapter(UserManager userManager, Context context) {
        ...
        boolean hasMultiAppPrifile = false;
        for (UserHandle profile : userProfiles) {
            if (profile.getIdentifier() == 999) {
                hasMultiAppPrifile = true;
                break;
            }
        }
        if ((hasMultiAppPrifile && userProfiles.size() < 3) || (!hasMultiAppPrifile && userProfiles.size() < 2)) {
            return null;
        }
    ...
    public static UserAdapter createUserAdapter(UserManager userManager, Context context, List<UserHandle> userProfiles) {
        ...
        boolean notShowProfiles = false;
        for (UserInfo userInfo : userManager.getProfiles(context.getUserId())) {
            if (userInfo.id == 999) {
                notShowProfiles = true;
                break;
            }
        }
        if (notShowProfiles) {
