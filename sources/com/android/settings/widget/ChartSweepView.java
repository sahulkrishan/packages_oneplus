...
public class ChartSweepView extends View {
    ...
    public ChartSweepView(Context context, AttributeSet attrs, int defStyle) {
        ...
        int color = a.getColor(1, -16776961);
        setSweepDrawable(a.getDrawable(6), color);
        setFollowAxis(a.getInt(0, -1));
        setNeighborMargin((float) a.getDimensionPixelSize(4, 0));
        setSafeRegion(a.getDimensionPixelSize(5, 0));
        setLabelMinSize(a.getDimensionPixelSize(2, 0));
        setLabelTemplate(a.getResourceId(3, 0));
        setLabelColor(color);
    ...
    private void invalidateLabelTemplate() {
        if (this.mLabelTemplateRes != 0) {
            ...
            this.mLabelLayout = Builder.obtain(this.mLabelTemplate, paint, 1024).setAlignment(Alignment.ALIGN_RIGHT).setIncludePad(false).setUseLineSpacingFromFallbacks(true).build();
            invalidateLabel();
        } else {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case 2:
                ...
                float clampedTargetY;
                if (this.mFollowAxis == 1) {
                    currentTargetY = (float) (getTop() - this.mMargins.top);
                    clampedTargetY = MathUtils.constrain(this.mTrackingStart + (event.getRawY() - this.mTracking.getRawY()), (float) clampRect.top, (float) clampRect.bottom);
                    setTranslationY(clampedTargetY - currentTargetY);
                    value = this.mAxis.convertToValue(clampedTargetY - ((float) parentContent.top));
                } else {
                    currentTargetY = (float) (getLeft() - this.mMargins.left);
                    clampedTargetY = MathUtils.constrain(this.mTrackingStart + (event.getRawX() - this.mTracking.getRawX()), (float) clampRect.left, (float) clampRect.right);
                    setTranslationX(clampedTargetY - currentTargetY);
                    value = this.mAxis.convertToValue(clampedTargetY - ((float) parentContent.left));
                }
    ...
    private Rect buildClampRect(Rect parentContent, long afterValue, long beforeValue, float margin) {
        ...
        if (!(beforeValue == Long.MIN_VALUE || beforeValue == Long.MAX_VALUE)) {
            z = true;
        }
        boolean beforeValid = z;
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (!isEnabled() || this.mLabelLayout == null) {
            ...
            sweepHeight = this.mSweep.getIntrinsicHeight();
            templateHeight = this.mLabelLayout.getHeight();
            this.mSweepOffset.x = 0;
            this.mSweepOffset.y = 0;
        ...
        if (this.mFollowAxis == 1) {
            sweepHeight = (this.mSweep.getIntrinsicHeight() - this.mSweepPadding.top) - this.mSweepPadding.bottom;
            this.mMargins.top = -(this.mSweepPadding.top + (sweepHeight / 2));
            this.mMargins.bottom = 0;
            this.mMargins.left = -this.mSweepPadding.left;
            this.mMargins.right = this.mSweepPadding.right;
        } else {
            sweepHeight = (this.mSweep.getIntrinsicWidth() - this.mSweepPadding.left) - this.mSweepPadding.right;
            this.mMargins.left = -(this.mSweepPadding.left + (sweepHeight / 2));
        ...
        sweepHeight = getMeasuredWidth();
        templateHeight = getMeasuredHeight();
        int widthAfter;
        int offset;
        Rect rect;
        if (this.mFollowAxis == 0) {
            widthAfter = sweepHeight * 3;
            setMeasuredDimension(widthAfter, templateHeight);
            this.mContentOffset.left = (widthAfter - sweepHeight) / 2;
            ...
            offset = templateHeight * 2;
            setMeasuredDimension(sweepHeight, offset);
            this.mContentOffset.offset(0, (offset - templateHeight) / 2);
            widthAfter = this.mSweepPadding.right * 2;
            rect = this.mContentOffset;
            rect.right -= widthAfter;
            rect = this.mMargins;
            rect.right += widthAfter;
        }
    ...
    public void onDraw(Canvas canvas) {
        ...
        count = labelSize;
        if (this.mFollowAxis == 1) {
            this.mSweep.setBounds(count, this.mSweepOffset.y, this.mContentOffset.right + width, this.mSweepOffset.y + this.mSweep.getIntrinsicHeight());
        } else {
            this.mSweep.setBounds(this.mSweepOffset.x, count, this.mSweepOffset.x + this.mSweep.getIntrinsicWidth(), this.mContentOffset.bottom + height);
        }
