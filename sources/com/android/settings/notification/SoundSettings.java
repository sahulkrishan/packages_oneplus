...
public class SoundSettings extends DashboardFragment implements OnDialogListItemClickListener, OnPreferenceChangeListener {
    ...
    public static final BaseSearchIndexProvider SEARCH_INDEX_DATA_PROVIDER = new BaseSearchIndexProvider() {
        ...
        public List<String> getNonIndexableKeys(Context context) {
            ...
            if (context.getResources().getBoolean(17956916)) {
                try {
            ...
            if (!OPUtils.isAppPakExist(context, "com.oneplus.sound.tuner")) {
                keys.add(SoundSettings.KEY_SOUND_TUNER);
            }
    ...
    private Preference mOPSoundTunerPreference;
        ...
        public void setListening(boolean listening) {
            ...
            if (OPUtils.isGuestMode()) {
                ...
                if (OPUtils.isSupportSocTriState()) {
                    ...
                    String ringtoneSummary = this.mContext.getString(R.string.op_ringtone).toLowerCase();
                    this.mSummaryLoader.setSummary(this, this.mContext.getString(R.string.join_many_items_middle, new Object[]{summary, ringtoneSummary}));
                    return;
                }
                summary = this.mContext.getString(R.string.oneplus_earphone_mode).toLowerCase();
                earphoneModeSummary = this.mContext.getString(R.string.op_ringtone).toLowerCase();
                this.mSummaryLoader.setSummary(this, this.mContext.getString(R.string.join_many_items_middle, new Object[]{summary, earphoneModeSummary}));
            }
        }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        this.mOPSoundTunerPreference = findPreference(KEY_SOUND_TUNER);
        this.mEarphoneModePreference = findPreference(KEY_EARPHONE_MODE);
        findPreference(KEY_EARPHONE);
        ...
        if (!(OPUtils.isAppPakExist(getActivity(), "com.oneplus.dirac.simplemanager") || this.mOPSoundDirectPreference == null)) {
            this.mOPSoundDirectPreference.setVisible(false);
        }
        if (!(OPUtils.isAppPakExist(getActivity(), "com.oneplus.sound.tuner") || this.mOPSoundTunerPreference == null)) {
            this.mOPSoundTunerPreference.setVisible(false);
        }
        if (OPUtils.isBlackModeOn(getContentResolver())) {
            this.mOPSoundTunerPreference.setIcon((int) R.drawable.op_ic_settings_dolby_white);
        } else {
            this.mOPSoundTunerPreference.setIcon((int) R.drawable.op_ic_settings_dolby_black);
        }
        if (OPUtils.isGuestMode()) {
    ...
    L_0x0017:
        r2 = 17040839; // 0x10405c7 float:2.4248716E-38 double:8.419293E-317;
        r8 = r10.getString(r2);
    ...
    private void updateDoNotDisturbPreference() {
        boolean z = false;
        int zen = Global.getInt(this.mContext.getContentResolver(), "Global.ZEN_MODE_CAR", 0);
    ...
    private void updateEffectsSuppressor() {
        ...
        if (!Objects.equals(suppressor, this.mSuppressor)) {
            ...
            if (this.mRingOrNotificationPreference != null) {
                String text;
                if (suppressor != null) {
                    text = this.mContext.getString(17040364, new Object[]{getSuppressorCaption(suppressor)});
                } else {
                    text = null;
                }
    ...
    private void updateVolumeKeysAdjustSummary(int value) {
        ...
        this.mVolumeKeysAdjust.setSummary(getContext().getResources().getStringArray(2130903250)[value]);
    }
    ...
    private void updateVibratePreferenceDescription(String key, int value) {
        ...
        if (incomingCallVibrateModePreference != null) {
            ...
            if (OPUtils.isSupportXVibrate()) {
                value = OPVibrateUtils.getRealXVibrateValueToIndex(value);
                entries = getActivity().getResources().getStringArray(R.array.incoming_call_x_vibrate_mode);
            } else {
                entries = getActivity().getResources().getStringArray(R.array.incoming_call_vibrate_mode);
            }
            incomingCallVibrateModePreference.setSummary(entries[value]);
        }
    }

    public boolean onPreferenceTreeClick(Preference preference) {
        String key = preference.getKey();
        if (KEY_INCOMING_CALL_VIBRATE.equals(key)) {
            String[] entriesValue;
            String[] entries;
            if (OPUtils.isSupportXVibrate()) {
                entriesValue = this.mContext.getResources().getStringArray(R.array.incoming_call_x_vibrate_mode_values);
                entries = this.mContext.getResources().getStringArray(R.array.incoming_call_x_vibrate_mode);
            } else {
                entriesValue = this.mContext.getResources().getStringArray(R.array.incoming_call_vibrate_mode_values);
                entries = this.mContext.getResources().getStringArray(R.array.incoming_call_vibrate_mode);
            }
            this.mOPListDialog = new OPListDialog(this.mContext, preference.getTitle(), entriesValue, entries);
            ...
            return true;
        } else if (!KEY_SOUND_TUNER.equals(key)) {
    ...
    public void OnDialogListItemClick(int index) {
        OPVibrateUtils.startVibrateByIndex(this.mVibrator, index);
    }

    public void OnDialogListConfirmClick(int index) {
        if (OPUtils.isSupportXVibrate()) {
            index = OPVibrateUtils.getRealXVibrateIndexToValue(index);
        }
