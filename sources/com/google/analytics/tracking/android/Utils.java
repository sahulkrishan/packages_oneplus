...
class Utils {
    ...
    public static String filterCampaign(String campaign) {
        ...
        urlParts = new String[]{"dclid", "utm_source", "gclid", "utm_campaign", "utm_medium", "utm_term", "utm_content", "utm_id", "gmob_t"};
        StringBuilder params = new StringBuilder();
        for (int i = 0; i < urlParts.length; i++) {
            if (!TextUtils.isEmpty((CharSequence) paramsMap.get(urlParts[i]))) {
                if (params.length() > 0) {
                    params.append("&");
                }
                params.append(urlParts[i]);
                params.append("=");
                params.append((String) paramsMap.get(urlParts[i]));
            }
        }
    ...
    static byte[] hexDecode(String s) {
        ...
        for (int i = 0; i < bytes.length; i++) {
            bytes[i] = (byte) ((fromHexDigit(s.charAt(2 * i)) << 4) | fromHexDigit(s.charAt((2 * i) + 1)));
        }
        return bytes;
    }
