...
public class OPApplicationLoader {
    ...
    public static final String PACKAGENAME_NETFLIX = "com.netflix.mediaclient";
    public static final String PACKAGENAME_ONEPLUS_CARMODE = "com.oneplus.carmode";
    public static final String PACKAGENAME_ONEPLUS_SPEECHASSIST = "com.oneplus.speechassist";
    public static final String PACKAGENAME_OP_CALENDAR = "com.oneplus.calendar";
    public static final String PACKAGENAME_PRIME_VIDEO = "com.amazon.avod.thirdpartyclient";
    ...
    private Map<String, String> mSelectedAppMap = new HashMap();
    ...
    public Map<String, String> loadSelectedGameOrReadAppMap(int type) {
                ...
                if (isThisUserAProfileOfCurrentUser(userId)) {
                    for (OpEntry op : packageOp.getOps()) {
                        if (op.getOp() == type && op.getMode() == 0) {
                            Map map = this.mSelectedAppMap;
                            StringBuilder stringBuilder = new StringBuilder();
                            stringBuilder.append(appUid);
                            stringBuilder.append(packageOp.getPackageName());
                            map.put(stringBuilder.toString(), packageOp.getPackageName());
                        }
                    }
                }
            }
                ...
                for (ResolveInfo reInfo : resolveInfos) {
                    ...
                    if (!noSystemApp || !PackageUtils.isSystemApplication(this.mContext, packageName)) {
                        if (multiAppPackageExcludeFilter(this.mContext, packageName)) {
                            ...
                            boolean z = map.containsKey(stringBuilder.toString()) && this.mSelectedAppMap.containsValue(packageName);
        ...
        try {
            ...
            if (!resolveInfos.isEmpty()) {
                for (ResolveInfo reInfo : resolveInfos) {
                    ...
                    String packageName = reInfo.activityInfo.packageName;
                    String appLabel = (String) reInfo.loadLabel(this.mPackageManager);
                    if (this.mAppType == 80 || !CAMERA_PACKAGE_NAME.equals(packageName)) {
                        if (this.mAppType == 80) {
                            String[] recommendedapps = this.mContext.getResources().getStringArray(R.array.op_car_mode_recommended_apps);
                            boolean IsRecommendedApp = false;
                            for (String equals : recommendedapps) {
                                if (equals.equals(packageName)) {
                                    IsRecommendedApp = true;
                                    break;
                                }
                            }
                            if (IsRecommendedApp) {
                        ...
                        OPAppModel appModel = new OPAppModel(packageName, appLabel, "", reInfo.activityInfo.applicationInfo.uid, false);
                        appModel.setAppIcon(getBadgedIcon(this.mPackageManager, reInfo));
            ...
            if (!resolveInfos.isEmpty()) {
                for (ResolveInfo reInfo : resolveInfos) {
                    ...
                    if (!packageExcludeFilter(packageName)) {
                        ...
                        stringBuilder.append(uid);
                        stringBuilder.append(packageName);
                        boolean z = (map.containsKey(stringBuilder.toString()) && this.mSelectedAppMap.containsValue(packageName)) ? true : i;
    ...
    private boolean packageExcludeFilter(String packageName) {
        ...
        if (this.mAppType != 63) {
            ...
            if (!(filter || SOUNDRECORDER_PACKAGE_NAME.equals(packageName) || CAMERA_PACKAGE_NAME.equals(packageName) || PACKAGENAME_YOUTUBE.equals(packageName) || PACKAGENAME_NETFLIX.equals(packageName) || PACKAGENAME_PRIME_VIDEO.equals(packageName))) {
                z = false;
            }
            return z;
        }
        if (!(false || PACKAGENAME_ONEPLUS_CARMODE.equals(packageName) || "com.oneplus.speechassist".equals(packageName) || "com.oneplus.deskclock".equals(packageName) || "com.android.settings".equals(packageName) || "com.google.android.googlequicksearchbox".equals(packageName))) {
            z = false;
        }
        return z;
    }
