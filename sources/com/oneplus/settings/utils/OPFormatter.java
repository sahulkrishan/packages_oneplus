...
public final class OPFormatter {
    ...
    private static class RoundedBytesResult {
        ...
        static RoundedBytesResult roundBytes(long sizeBytes, int flags) {
            ...
            int roundFactor2 = roundFactor;
            int roundDigits = i;
            if (isNegative) {
                result = -result;
            }
            if ((flags & 2) != 0) {
                j2 = (((long) Math.round(((float) roundFactor2) * result)) * mult) / ((long) roundFactor2);
            }
            return new RoundedBytesResult(result, units, roundDigits, j2);
        }
    }
    ...
    private static String getSuffixOverride(Resources res, MeasureUnit unit) {
        if (unit == MeasureUnit.BYTE) {
            return res.getString(17039595);
        }
        return res.getString(17040717);
    }
    ...
    private static String deleteFirstFromString(String source, String toDelete) {
        ...
        stringBuilder.append(source.substring(0, location));
        stringBuilder.append(source.substring(toDelete.length() + location, source.length()));
    ...
    private static String formatRoundedBytesResult(Context context, RoundedBytesResult input) {
        ...
        return context.getString(17039924, new Object[]{numberFormatter.format((double) input.value), getSuffixOverride(context.getResources(), input.units)});
    }
