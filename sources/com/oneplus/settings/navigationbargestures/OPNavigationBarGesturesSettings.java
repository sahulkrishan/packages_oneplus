...
public class OPNavigationBarGesturesSettings extends SettingsPreferenceFragment implements OnClickListener, OnPreferenceChangeListener, Indexable {
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (getBackHomeEnabled()) {
            this.mEnterStatus = KEY_BACK_HOME;
            return;
        }
        int value = System.getInt(this.mContext.getContentResolver(), KEY_OP_NAVIGATION_BAR_TYPE, 1);
        if (value == 1) {
            this.mEnterStatus = KEY_ALWAYS_SHOW_NAVIGATION_BAR;
        } else if (value == 3) {
            this.mEnterStatus = KEY_GESTURE_NAVIGATION_BAR;
        }
    }
    ...
    public void onDestroy() {
        ...
        String mExitStatus = KEY_GESTURE_NAVIGATION_BAR;
        if (getBackHomeEnabled()) {
            OPUtils.sendAnalytics("nav_gesture", NotificationCompat.CATEGORY_STATUS, "2");
            mExitStatus = KEY_BACK_HOME;
        } else {
            int value = System.getInt(this.mContext.getContentResolver(), KEY_OP_NAVIGATION_BAR_TYPE, 1);
            if (value == 1) {
                OPUtils.sendAnalytics("nav_gesture", NotificationCompat.CATEGORY_STATUS, "1");
                mExitStatus = KEY_ALWAYS_SHOW_NAVIGATION_BAR;
            } else if (value == 3) {
                OPUtils.sendAnalytics("nav_gesture", NotificationCompat.CATEGORY_STATUS, "3");
                mExitStatus = KEY_GESTURE_NAVIGATION_BAR;
            }
        }
        if (KEY_ALWAYS_SHOW_NAVIGATION_BAR.equalsIgnoreCase(this.mEnterStatus) && KEY_GESTURE_NAVIGATION_BAR.equalsIgnoreCase(mExitStatus)) {
            OPUtils.sendAnalytics("nav_gesture", "resource", "full_screen_default");
        } else if (KEY_BACK_HOME.equalsIgnoreCase(this.mEnterStatus) && KEY_GESTURE_NAVIGATION_BAR.equalsIgnoreCase(mExitStatus)) {
            OPUtils.sendAnalytics("nav_gesture", "resource", "full_screen_capsule");
        } else if (KEY_GESTURE_NAVIGATION_BAR.equalsIgnoreCase(this.mEnterStatus) && KEY_ALWAYS_SHOW_NAVIGATION_BAR.equalsIgnoreCase(mExitStatus)) {
            OPUtils.sendAnalytics("nav_gesture", "resource", "default_full_screen");
        } else if (KEY_GESTURE_NAVIGATION_BAR.equalsIgnoreCase(this.mEnterStatus) && KEY_BACK_HOME.equalsIgnoreCase(mExitStatus)) {
