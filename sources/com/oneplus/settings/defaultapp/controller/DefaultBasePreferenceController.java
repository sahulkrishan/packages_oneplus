...
public abstract class DefaultBasePreferenceController extends DefaultAppPreferenceController {
    ...
    private CharSequence getOnlyAppLabel() {
        ...
        List<DefaultAppInfo> list = getCandidates();
        if (list == null || list.size() != 1) {
            return null;
        }
        DefaultAppInfo info = (DefaultAppInfo) list.get(0);
        String str = TAG;
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("Getting label for the only app: ");
        stringBuilder.append(info.componentName);
